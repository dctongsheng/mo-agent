...
def _parse_test_cases(text: str) -> list:
    """Best-effort parse of an LLM-produced JSON array of test cases.

    Handles: code fences, leading/trailing prose, and a malformed array where
    only some objects are valid (salvages the good ones via a balanced-brace
    scan instead of failing the whole batch).
    """
    if not text:
        return []
    s = text.strip()
    ...
    if s.startswith("```"):
        s = re.sub(r"^```[a-zA-Z]*\n?", "", s)
        s = re.sub(r"\n?```$", "", s).strip()
    # 1) whole-string parse (JSON, then Python-literal for single-quoted dicts)
    for loader in (json.loads, _py_literal):
        try:
            v = loader(s)
            ...
            pass
    # 2) first [...] block (JSON or Python-literal)
    ...
    if m:
        for loader in (json.loads, _py_literal):
            try:
                ...
                pass
    # 3) salvage individual {...} objects via balanced-brace scan
    objs = []
    depth = 0
    start = -1
    in_str = False
...
def _py_literal(text: str):
    """Parse a Python literal (e.g. single-quoted dicts the model sometimes
    emits instead of strict JSON). Safe — no code execution."""
    import ast
...
class SyntheticDatasetBuilder:
    ...
    def generate(
        ...
        # Parse the generated test cases. Non-GPT models (qwen, etc.) often
        # wrap JSON in code fences or emit a slightly malformed array, so parse
        # defensively and salvage individual objects rather than failing the
        # whole run on one bad delimiter.
